...
/* ── Steps 2-4: pick up ?code&state, exchange via backend ── */
async function consumeAuthCode() {
  ...
  // Clean the URL immediately so a refresh doesn't replay the code
  ...
  if (!verifier || !state || state !== savedState) {
    setConnectBtnLoading(false);
    showToast('❌ Login state mismatch. Please try again.');
    return false;
  }
  ...
  try {
    ...
    sessionStorage.setItem('db_access_token', STATE.accessToken);
    sessionStorage.setItem('db_token_expiry', String(STATE.tokenExpiry));
    if (data.refresh_token) sessionStorage.setItem('db_refresh_token', data.refresh_token);
    openTradingSocket(CONFIG.WS_URL + '?app_id=' + CONFIG.APP_ID, STATE.accessToken, 'pkce');
    return true;
  } catch (err) {
    setConnectBtnLoading(false);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 1. Deriv just redirected back with ?code&state — exchange it via the backend
  // 2. A previous session token is still valid in sessionStorage
